...
export function PublicLanding({ onStart, onDemo }: { onStart: () => void, onDemo: () => void }) {
  return <main className="landing-page">
    ...
    <section className="landing-hero warm-hero" id="home">
...
function InvestorLanding({ onStart, onDemo }: { onStart: () => void, onDemo: () => void }) {
  ...
  return <main className="landing-page">
    <header className="landing-nav warm-nav">
      <Logo onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} />
      <nav aria-label="Public navigation" className="nav-menu">
        <a href="#home">Home</a>
        <div className="nav-dropdown"><a href="#about">About us</a><div className="dropdown-panel"><a href="#about">Why Navigator</a><a href="#data-engine">Data approach</a><button onClick={onDemo}>See working demo</button></div></div>
        <div className="nav-dropdown"><a href="#services">Services</a><div className="dropdown-panel"><a href="#services">Study abroad</a><a href="#services">Career planning</a><a href="#services">Job preparation</a><a href="#services">Job search</a></div></div>
        <div className="nav-dropdown"><a href="#community">Community</a><div className="dropdown-panel"><a href="#community">Student support</a><a href="#journeys">Success stories</a><button onClick={onStart}>Join community</button></div></div>
        <div className="nav-dropdown"><a href="#plans">Plans</a><div className="dropdown-panel"><a href="#plans">Free discovery</a><a href="#plans">Guided plans</a><button onClick={onDemo}>Compare in demo</button></div></div>
      </nav>
    ...
    <section className="landing-hero warm-hero" id="home">
      <div className="hero-copy"><span className="eyebrow">AI guidance for study, career and jobs</span><h1>A warmer, clearer path from <em>confused</em> to ready.</h1><p>Project Navigator listens to a user’s background, then creates a practical roadmap with recommendations, documents, expert help and progress monitoring.</p><div className="landing-actions"><button className="primary" onClick={onDemo}>Explore the working demo -&gt;</button><button className="secondary" onClick={onStart}>Create my profile</button></div><div className="hero-trust"><span>For students</span><span>For job seekers</span><span>For career switchers</span></div><small>Prototype uses fixed mock data. No payment required.</small></div>
      <div className="hero-visual" aria-label="Product preview"><img src={heroImage} alt="Student planning future goals with guidance" /><div className="floating-card readiness-card"><small>Samira's readiness</small><strong>74%</strong><span>Next: IELTS + funding proof</span></div><div className="landing-preview mini-preview"><div className="preview-head"><span>Guided route</span><strong>Live demo</strong></div><div className="preview-route"><span className="done">✓</span><div><strong>Profile mapped</strong><small>Academic, budget and destination signals</small></div></div><div className="preview-route"><span className="active">2</span><div><strong>3 strong matches</strong><small>Program fit + funding gap detected</small></div></div><div className="preview-match"><small>Strongest current match</small><strong>Technical University of Munich</strong><span>88% profile fit</span></div></div></div>
    </section>
